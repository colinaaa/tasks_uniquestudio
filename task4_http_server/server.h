...
#define EXIT_IF(r, ...)                                                        \
  if (r) {                                                                     \
    printf(__VA_ARGS__);                                                       \
    printf("%s:%d\n, errno:%d, msg:%s", __FILE__, __LINE__, errno,             \
           strerror(errno));                                                   \
    exit(1);                                                                   \
  }
...
public:
  ...
  int handleEvent();
  virtual int getHandle() { return _socketAcceptor.get_fd(); }
};
...
public:
  ...
  virtual ~readHandler(){};
  int handleEvent();
  int getHandle() { return peerStream.get_fd(); }
...
public:
  ...
  int handleEvent();
  virtual int getHandle() { return peerStream.get_fd(); }
